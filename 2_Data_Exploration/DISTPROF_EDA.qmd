...
hist <- ggplot(demographic_data, aes(x = .data[[demographic]])) +
  ...
  scale_x_continuous(labels = scales::label_wrap(18)) +
...
geo_tx <- read_sf("0_Datasets/Geometry/Texas_SchoolDistricts_2024.geojson")

geo_tx$DISTRICT_N <- as.integer(geo_tx$DISTRICT_N)

geo_htx <- st_transform(read_sf("0_Datasets/Geometry/Houston_SchoolDistricts_2019.geojson"))

geo_htx$DISTRICT_N <- as.integer(geo_htx$DISTRICT_N)

distprof_nocharter <- distprof_joined %>% filter(Charter.School..Y.N. == "N" & Year == 2023)
```
...
```{r}
binning <- function(x) {
  quantiles <- unique(round(quantile(x, probs = seq(0, 1, 0.2), na.rm = TRUE), 0))

  if (length(quantiles) < 2) {
    stop("Not enough unique bins to create categories")
  }

  # Ensure 0 and 100 are included if missing
  if (min(quantiles) > 0) {
    quantiles <- c(0, quantiles)
  }
  if (max(quantiles) < 100) {
    quantiles <- c(quantiles, 100)
  }

  # Ensure breakpoints are strictly increasing
  breaks <- sort(unique(quantiles))

  # Create labels
  labels <- paste0(breaks[-length(breaks)], "-", breaks[-1], "%")

  if (min(breaks) > 0) {
    labels <- c(paste0("<", breaks[2], "%"), labels)
  }
  if (max(breaks) < 100) {
    labels <- c(labels, paste0(">", breaks[length(breaks) - 1], "%"))
  }

  # Ensure labels and breaks align
  if (length(labels) != length(breaks) - 1) {
    stop("Labels and breaks are not the same length")
  }

  bins <- cut(x, breaks = breaks, labels = labels, include.lowest = TRUE)
...
```{r}
generate_demographic_map <- function(year, include_charter = FALSE, level = "State", demographic) {
  demographic_key <- list(
  "Student_Membership_White_Percent" = "White Student Percent",
  "Student_Membership_African_American_Percent" = "Black Student Percent",
  "Student_Membership_Hispanic_Percent" = "Hispanic Student Percent",
  ...
  demographic_data <- year_distprof %>% select(all_of(c("DISTRICT", "DISTNAME", demographic)))
  
  if (level == "Houston") {
    demographic_with_geo <- demographic_data %>% inner_join(geo_htx, join_by("DISTRICT" == "DISTRICT_N"))
    
    demographic_with_geo$bins <- binning(demographic_with_geo[[demographic]])
    
    map <- ggplot() + 
  geom_sf(data = geo_tx, fill = "gray90", color = "grey20", lwd = 0.2) +

  geom_sf(data = demographic_with_geo, aes(geometry = geometry, fill = bins)) +
    
 labs(title = paste0(demographic_key[[demographic]], " in Houston Area School Districts, ", year),
       x = demographic_key[[demographic]],
       y = "Count") +
  ...
  if (level == "State") {
    demographic_with_geo <- demographic_data %>% inner_join(geo_tx, join_by("DISTRICT" == "DISTRICT_N"))
 ...
 labs(title = paste0(demographic_key[[demographic]], " in Texas School Districts, ", year),
      fill = demographic_key[[demographic]]) +
  ...
  else stop("Invalid input for level")
...
```{r}
generate_demographic_map(2023, demographic = "Student_Membership_African_American_Percent")
```